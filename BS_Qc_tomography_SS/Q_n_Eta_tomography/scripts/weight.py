...
lonA=float(input("Enter the lowest longitude:"))
lonB=float(input("Enter the highest longitude:"))
latA=float(input("Enter the lowest latitude:"))
latB=float(input("Enter the highest longitude:"))
grdsz=float(input("Enter the gridsize:")) 

xran = np.arange(lonA, lonB, grdsz)
yran = np.arange(latA, latB, grdsz)
...
#--------------------------------------------------
fopen = open("cata_sorted_l1000.txt")
...
for line in fopen:
    if line[0] != "#":
       ...
       Slong.append(float(line.split()[6]))
       Slat.append(float(line.split()[5]))
       Rlat.append(float(line.split()[7]))
       Rlong.append(float(line.split()[8]))
       ts2.append(float(line.split()[9]))
       dist.append(float(line.split()[10]))
       dep.append(float(line.split()[11]))
       Q0.append(float(line.split()[1]))
       eta.append(float(line.split()[3]))
       data.append(line.split()[12:])
fopen.close()
...
for i in range(len(midlat)):
    ...
    tmpy = []
    elchk = []
    tick = np.arange(total_points)*2.0/float(total_points)
    ...
    fopen.write("\t".join("%013.010f"%(c) for c in gridwt)+"\t%013.010f"%su+"\n")
